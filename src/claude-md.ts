...
export function generateBlock(db: Database, projectPath: string): string {
  ...
  const deadEnds = getDeadEndsByProject(db, projectPath).filter(de => !de.resolved);
  ...
  const contextTypes = ['pattern', 'gotcha', 'progress', 'context', 'architecture'] as const;
  ...
  if (checkpoint) {
    ...
    if (checkpoint.blockers) cpLines.push(`- **Blockers:** ${checkpoint.blockers}`);
