...
describe('callClaude', () => {
  it('returns parsed JSON result field', async () => {
    const proc = createMockProcess(JSON.stringify({ result: 'Hello from Claude' }), '', 0);
...
describe('callClaudeJson', () => {
  it('returns typed JSON', async () => {
    ...
    const proc = createMockProcess(JSON.stringify({ result: JSON.stringify(data) }), '', 0);
    mockSpawn.mockReturnValue(proc as ReturnType<typeof spawn>);
  ...
  it('throws on invalid JSON response', async () => {
    const proc = createMockProcess(JSON.stringify({ result: 'not json at all' }), '', 0);
    mockSpawn.mockReturnValue(proc as ReturnType<typeof spawn>);

    await expect(callClaudeJson('test')).rejects.toThrow('Failed to parse JSON');
  });
});
